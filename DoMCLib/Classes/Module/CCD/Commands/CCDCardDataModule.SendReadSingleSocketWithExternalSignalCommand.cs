...
namespace DoMCLib.Classes.Module.CCD
{
    ...
    {
        public class SendReadSingleSocketWithExternalSignalCommand : WaitingCommandBase
        {
            CCDCardDataCommandResponse result = new CCDCardDataCommandResponse();
            int cardNumber = -1;
            public SendReadSingleSocketWithExternalSignalCommand(IMainController mainController, AbstractModuleBase module) : base(mainController, module, typeof((DoMCApplicationContext Context, int EquipmentSocketNumber)), typeof(CCDCardDataCommandResponse)) { }
            protected override void Executing()
            {
                ...
                if (contextSocket != null && contextSocket.Value.Context != null)
                {
                    ...
                    module.tcpClients[cardNumber].SendCommandReadSeveralSocketsExternal(CancelationTokenSourceToCancelCommandExecution.Token);
                }
                else
                {
                }
