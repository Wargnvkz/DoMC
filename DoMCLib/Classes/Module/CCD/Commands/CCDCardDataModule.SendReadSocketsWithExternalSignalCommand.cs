...
namespace DoMCLib.Classes.Module.CCD
{
    ...
    {
        public class SendReadSocketsWithExternalSignalCommand : WaitingCommandBase
        {
            CCDCardDataCommandResponse result = new CCDCardDataCommandResponse();
            public SendReadSocketsWithExternalSignalCommand(IMainController mainController, AbstractModuleBase module) : base(mainController, module, typeof(DoMCApplicationContext), typeof(CCDCardDataCommandResponse)) { }
            protected override void Executing()
            {
                ...
                if (context != null)
                {
                    ...
                    for (int i = 0; i < cardParameters.Count; i++)
                    {
                        ...
                        module.tcpClients[cardParameters[i].Item1].SendCommandReadSeveralSocketsExternal(CancelationTokenSourceToCancelCommandExecution.Token);
                    }
                }
                else
                {
